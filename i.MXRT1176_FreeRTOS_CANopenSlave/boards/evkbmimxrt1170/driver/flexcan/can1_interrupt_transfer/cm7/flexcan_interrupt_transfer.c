...
static void CANopenRx_Task(void* parameter)
{
    while (1) {
        ...
        (void)FLEXCAN_TransferReceiveNonBlocking(EXAMPLE_CAN, &flexcanHandle, &rxXfer);
				...
				LOG_INFO("Node 2\r\n");
				//LOG_INFO("Rx MB ID: 0x%3x,\r\n", rxXfer.frame->id >> CAN_ID_STD_SHIFT);
				rxMessage.cob_id = (UNS16)(rxXfer.frame->id >> CAN_ID_STD_SHIFT);
				//LOG_INFO("Rx MB ID: 0x%3x,\r\n", rxMessage.cob_id);
//				if (rxXfer.frame->format == kFLEXCAN_FrameFormatStandard) 
//				{
//					rxMessage.cob_id = (UNS16)(rxXfer.frame->id & 0x7FF); 
//				} 
//				else {
//						rxMessage.cob_id = (UNS16)(rxXfer.frame->id & 0x1FFFFFFF);
//				}
				...
				LOG_INFO("Node 3\r\n");
				canDispatch(&Slave_Data, &rxMessage);
				LOG_INFO("Node 4\r\n");
				
				for (int i = 0; i < rxMessage.len; i++) 
				{
					LOG_INFO("Rx MB ID: 0x%3x, Rx MB data: 0x%x, Time stamp: %d\r\n", rxMessage.cob_id,
                 rxMessage.data[i], rxXfer.frame->timestamp);
				}
        }
}
